...
**Access is enforced differently per agent, and the flags are load-bearing.** Fable and Opus are native Claude subagents held read-only by their instructions alone — their tool lists include `Bash`, so nothing mechanical stops them. Sol is confined by `--sandbox read-only`; Luna by `--sandbox workspace-write`; `grok-implementer` by `--sandbox workspace`, which refuses writes to the repository outside its worktree while still allowing `~/.grok` and the temp dirs. `grok-reviewer` is confined twice over, by `--sandbox read-only` and by four deny rules. Both grok roles need `--deny MCPTool` on top of any sandbox, because MCP servers are separate processes a sandbox does not cover.
